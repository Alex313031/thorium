...
```
 ...
 - You must have the version 10.1.22621.2428 (or higher) [Windows 11
SDK](https://developer.microsoft.com/en-us/windows/downloads/sdk-archive/)
installed. This can be installed separately or by checking the appropriate box
in the Visual Studio Installer. __(Note that MSVS 2022 will try to install the
22000 version by default, uncheck this and check the 22621 version)__.

 - The 10.1.22621.2428 (or higher) SDK Debugging Tools must also be installed.
This version of the Debugging tools is needed in order to support reading the
large-page PDBs that Chrome uses to allow greater-than 4 GiB PDBs. This can be
installed after the matching Windows SDK version is installed, from: Control
Panel -> Programs and Features -> Windows Software Development Kit [version] ->
Change -> Debugging Tools for Windows. Or, you can download the standalone SDK
installer and use it to install the Debugging Tools. If building on ARM64
Windows then you will need to manually copy the Debuggers\x64 directory from
another machine because it does not get installed on ARM64 and is needed,
whether you are building Chromium for x64 or ARM64 on ARM64.
...
***
### Install `python`
To ensure that the `*.py` files in the Thorium repo can work properly, please
install [Python](https://www.python.org/downloads/) on the device, and it is
recommended to install Python 3.11 series version.

At the same time, please make sure that the environment variables configured in
the python you just installed are configured correctly.

***
### Install `7-zip`
**Optional:** To ensure that the `make_portable_win.py` files can work properly,
please make sure [7-zip](https://www.7-zip.org/download.html) is installed on
the device and the installation directory of 7-zip needs to be placed in the
...
- **Optional:** At the same time, you can also set the `NINJA_SUMMARIZE_BUILD`
environment variable and set it to __1__, which mainly collects various data at
compile time. It will show how many build processes are running at any given
time, how many build steps have completed, how many build steps have completed
per second, and how long the build has been running, and print a build
performance summary when the build completes, showing the slowest build steps
and slowest build-step types.
...
The following is an example of correct environment variable configuration, for
reference only, these environment variables are YMMV for everyone.

At the same time, the following environment variables are recommended to be set
in the **user-level** environment variables.

**These two environment variables are set separately.**
```
DEPOT_TOOLS_WIN_TOOLCHAIN = 0
NINJA_SUMMARIZE_BUILD = 1
vs2022_install = C:\Program Files\Microsoft Visual Studio\2022\Preview
```

**The following environment variables are set in the environment variable entry
`Path`.**
```
C:\Program Files\Git\cmd
C:\Program Files\Git\mingw64\bin
C:\Program Files\Git\usr\bin
C:\src\depot_tools
C:\Users\gz83\AppData\Local\Programs\Python\Python311\Scripts\
C:\Users\gz83\AppData\Local\Programs\Python\Python311\
D:\software\7-Zip
```
...
Using Git:
It should be placed in %USERPROFILE%, i.e. `C:\Users\$USERNAME`.   
...
- NOTE: To build for AVX2, use `setup.py --avx2`. To build SSE3, use `setup.py --sse3`. Use `setup.py --help` to see all options/platforms.
...
We will be copy/pasting the contents of the [win_args.gn](https://github.com/Alex313031/thorium/blob/main/win_args.gn) file (from *%USERPROFILE%\thorium\win_args.gn*) into notepad.
...
```shell
python win_scripts\build.py 24
```
(24 can be any number, and should generally be set to the number of cores on
your CPU. At the same time, the file can also read the number of threads of the
processor to automatically set the value.).

`win_scripts\build.py` compiles by calling `autoninja`, `autoninja` is a wrapper
that automatically provides optimal values for the arguments passed to `ninja`.
...
Or install it using the thorium_mini_installer:

```shell
out\thorium\thorium_mini_installer.exe
```

## Update your checkout

To update an existing Chromium checkout, you should run the `win_scripts\trunk.py` script again:
